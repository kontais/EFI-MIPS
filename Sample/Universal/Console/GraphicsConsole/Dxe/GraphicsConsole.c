...
--*/
{
  ...
  // of 1024x600, 16 bit colot, 60 Hz refresh.
  //
  ...
  if (Private->GraphicsOutput != NULL) {
    //
    // The console is build on top of Graphics Output Protocol, find the mode number for 1024x600
    //
  ...
  //
  // If the graphics mode is 1024x600, than add a text mode that uses the entire display
  //
  if (HorizontalResolution == 1024 && VerticalResolution == 600) {

    if (MaxMode < 2) {
      Private->ModeData[MaxMode].Columns    = 0;
      Private->ModeData[MaxMode].Rows       = 0;
      Private->ModeData[MaxMode].GopWidth   = 1024;
    ...
    Private->ModeData[MaxMode].Columns    = 1024 / GLYPH_WIDTH;
    Private->ModeData[MaxMode].Rows       = 600 / GLYPH_HEIGHT;
    Private->ModeData[MaxMode].GopWidth   = 1024;
    Private->ModeData[MaxMode].GopHeight  = 600;
    Private->ModeData[MaxMode].GopModeNumber = ModeNumber;
    Private->ModeData[MaxMode].DeltaX     = (1024 % GLYPH_WIDTH) >> 1;
