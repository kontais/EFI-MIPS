...
--*/
{
  ...
  // of 640x400, 16 bit colot, 60 Hz refresh.
  //
  ...
  if (Private->GraphicsOutput != NULL) {
    //
    // The console is build on top of Graphics Output Protocol, find the mode number for 640x400
    //
  ...
  //
  // If the graphics mode is 640x400, than add a text mode that uses the entire display
  //
  if (HorizontalResolution == 640 && VerticalResolution == 400) {

    if (MaxMode < 2) {
      Private->ModeData[MaxMode].Columns    = 0;
      Private->ModeData[MaxMode].Rows       = 0;
      Private->ModeData[MaxMode].GopWidth   = 640;
      Private->ModeData[MaxMode].GopHeight  = 400;
      Private->ModeData[MaxMode].GopModeNumber = ModeNumber;
    ...
    Private->ModeData[MaxMode].Columns    = 640 / GLYPH_WIDTH;
    Private->ModeData[MaxMode].Rows       = 400 / GLYPH_HEIGHT;
    Private->ModeData[MaxMode].GopWidth   = 640;
    Private->ModeData[MaxMode].GopHeight  = 400;
    Private->ModeData[MaxMode].GopModeNumber = ModeNumber;
    Private->ModeData[MaxMode].DeltaX     = (640 % GLYPH_WIDTH) >> 1;
    Private->ModeData[MaxMode].DeltaY     = (400 % GLYPH_HEIGHT) >> 1;
    MaxMode++;
  }
  //
  ...
  DEBUG_CODE (
    GraphicsConsoleConOutOutputString (&Private->SimpleTextOutput, L"Graphics Console Started\n\r");
  )
  //
  ...
  )
{
  ...
  *Foreground = mEfiColors[1];
  *Background = mEfiColors[0];
  ...
  )
{
  ...
  while (Index < Count) {
    ...
    EfiCopyMem (&GlyphData, Glyph, sizeof (GLYPH_UNION));
    ...
    do {
      ...
      if (GlyphWidth == 0x10 && Counts == 1) {
        EfiCopyMem (&GlyphData.NarrowGlyph.GlyphCol1, &Glyph->WideGlyph.GlyphCol2, sizeof (Glyph->WideGlyph.GlyphCol2));
      }
  ...
  if (GraphicsOutput != NULL) {
    ...
    UgaDraw->Blt (
              UgaDraw,
              (EFI_UGA_PIXEL *) Private->LineBuffer,
