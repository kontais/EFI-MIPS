  ...
  )
{
...
#if (EFI_SPECIFICATION_VERSION < 0x00020000)
  //
  // Setup the DevNullUgaDraw to 1024 x 600 x 16 bits per pixel
  //
  ConSpliterUgaDrawSetMode (&ConOutPrivate->UgaDraw, 1024, 600, 16, 60);
#else
  //
  ...
  //
  // Setup the DevNullGraphicsOutput to 1024 x 600 x 16 bits per pixel
  //
...
--*/
{
  ...
  if (GraphicsOutput != NULL) {
    ...
    //
    // Select a prefered Display mode 1024x600
    //
    for (Index = 0; Index < CurrentGraphicsOutputMode->MaxMode; Index++) {
      Mode = &Private->GraphicsOutputModeBuffer[Index];
      if ((Mode->HorizontalResolution == 1024) && (Mode->VerticalResolution == 600)) {
        break;
      }
    }
    //
