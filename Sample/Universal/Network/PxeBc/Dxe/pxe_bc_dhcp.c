  ...
  )
{
  ...
  if (LocalPtr.OpPtr && (LocalPtr.Overload->Overload & OVLD_FILE)) {
    ...
    RxBufPtr->OpAdds.PktOptAdds[OP_DHCP_BOOTFILE_IX - 1]->Header.Length = (UINT8) EfiAsciiStrLen (RxBufPtr->u.Dhcpv4.file);
  }
  ...
  )
{
  ...
  EfiCopyMem (
    ...
    &DHCPV4_ACK_BUFFER.OpAdds.PktOptAdds[OP_DHCP_SERVER_IP_IX - 1],
  ...
  )
{
  ...
  for (;;) {
    ...
    //
    if (!DhcpRxBuf->u.Dhcpv4.yiaddr && !EfiCompareMem (
                                          &ServerIpPtr->v4,
  ...
  for (Private->Timeout = 1;
    ...
    if (UseBis) {
      ...
      for (Index = 0; Index < Index2; ++Index) {
        ...
        EfiCopyMem (&x, &BisSigInfo[Index], sizeof x);
        x = HTONL (x);
        EfiCopyMem (&DISCREDoptions.Credentials[Index], &x, sizeof x);
      }
...
--*/
{
  ...
  if (DiscoverInfoPtr->UseUCast) {
    ...
    for (Index = 0; Index < ServerListPtr->Ipv4List.IpCount; ++Index) {
      EfiCopyMem (&DestIp, &ServerListPtr->Ipv4List.IpList[Index], 4);
