...
#if 0
下面的代码要复制到内存中其它位置运行，需要位置无关代码
但是-G 0 -mno-abicalls -fno-pic使用j指令，与位置相关
所以需要DxeIplLib的PeiImageRead.c实现，使用-fpic
但生成的.o中不可以用gp的使用
EFI_STATUS
PeiImageRead (
  IN     VOID    *FileHandle,
...
--*/
{
  ...
  ImageContext->ImageRead = (EFI_PEI_ELF_LOADER_READ_FILE) (UINTN) MemoryBuffer;
