...
UINT64                    mTimerCompare = 0xffffffff;
...
//
// Pointer to the Mips32Interrupt Protocol instance
//
EFI_MIPS32_INTRRUPT_PROTOCOL  *mMips32Intr;
...
Routine Description:
  ...
  Sets the counter value for mips32

Arguments:

  Count - The 32-bit counter value to program into mips32 count
...
Arguments:
  ...
  Count - The 32-bit counter value to program into mips32 compare
...
--*/
{
  ...
  //
  CP0_SetCompare(CP0_GetCount() + (UINT32) mTimerCompare);
...
{
  //
  //  The count inc rate is 797/2 MHz or 39.85 ticks per 100 ns.
  //  TimerPeriod * 39.85 = compare value. Using integer arithmetic
  //  mTimerCompare = (TimerPeriod * 3985)/100.
  //
  //  Round up to next highest integer. This guarantees that the timer is
  //  equal to or slightly longer than the requested time.
  //  mTimerCompare = ((TimerPeriod * 3985) + 50)/100
  //
  // Note that a mTimerCompare of 0 is equivalent to a count of 0xffffffff
  //
  // Since mTimerCompare is limited to 16 bits for IA32, TimerPeriod is limited
  // to 20 bits.
  //
  if (TimerPeriod == 0) {
    //
    // Disable compare interrupt for a TimerPeriod of 0
    //
    mMips32Intr->DisableIntr (mMips32Intr, EfiMips32Intr7);
  } else {
    //
    // Convert TimerPeriod into mips cp0 compare
    //
    mTimerCompare = DivU64x32 (MultU64x32 (3985, (UINTN) TimerPeriod) + 50, 100, 0);

    //
    // Check for overflow
    //
    if (mTimerCompare >= 0xffffffff) {
      mTimerCompare = 0;
    ...
    //
    // Program the mips32 timer with the new count value
    //
    SetCp0Count (0);
    SetCp0Compare ((UINT32) mTimerCompare);

    //
    // Enable timer interrupt
    //
    mMips32Intr->EnableIntr (mMips32Intr, EfiMips32Intr7);
  }
  //
...
--*/
{
  ...
  //
  Status = mMips32Intr->GetMask (mMips32Intr, &IntrMask);
  ASSERT_EFI_ERROR (Status);
  if ((IntrMask & (0x1 << EfiMips32Intr7)) != 0) {
    //
...
--*/
{
  ...
  //
  Status = gBS->LocateProtocol (&gEfiMips32InterruptProtocolGuid, NULL, (VOID **) &mMips32Intr);
  ASSERT_EFI_ERROR (Status);
  ...
  //
  // Get the interrupt vector number corresponding to MIPS32 Cause IP[7]
  //
  TimerVector = 0;
  Status      = mMips32Intr->GetVector (mMips32Intr, EfiMips32Intr7, &TimerVector);
  ASSERT_EFI_ERROR (Status);

  //
  // Install interrupt handler for MIPS32 Cause IP[7]
  //
