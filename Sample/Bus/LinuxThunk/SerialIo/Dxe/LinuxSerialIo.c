...
Abstract:
  ...
  The driver then adds a gEfiLinuxSerialPortGuid as a protocol to Handle(1).
  The instance data for this protocol is the private data used to create
  Handle(2).

  Handle(1) - LinuxIo - DevicePath(1) - LinuxSerialPort

  If the driver is unloaded Handle(2) is removed from the system and 
  gEfiLinuxSerialPortGuid is removed from Handle(1).
  ...
  a Handle(1) to contain a LinuxIo protocol, a DevicePath protocol, and 
  the TypeGuid in the LinuxIo must be gEfiLinuxSerialPortGuid. 
  
  If Handle(1) contains a gEfiLinuxSerialPortGuid protocol then the driver is 
...
EFI_DRIVER_BINDING_PROTOCOL gLinuxSerialIoDriverBinding = {
  LinuxSerialIoDriverBindingSupported,
  LinuxSerialIoDriverBindingStart,
  LinuxSerialIoDriverBindingStop,
  0x10,
  NULL,
  NULL
};
...
EFI_DRIVER_ENTRY_POINT (InitializeLinuxSerialIo)

EFI_STATUS
EFIAPI
InitializeLinuxSerialIo (
...
// TODO:    SystemTable - add argument and description to function comment
{
  return EfiLibInstallAllDriverProtocols (
          ImageHandle,
          SystemTable,
          &gLinuxSerialIoDriverBinding,
          ImageHandle,
          &gLinuxSerialIoComponentName,
          NULL,
          NULL
          );
}
...
// TODO:    EFI_SUCCESS - add return value to function comment
{
  ...
  //
  if (!EfiCompareGuid (LinuxIo->TypeGuid, &gEfiLinuxSerialPortGuid)) {
    Status = EFI_UNSUPPORTED;
    goto Error;
  }
...
--*/
{
  EFI_STATUS                          Status;
  EFI_LINUX_IO_PROTOCOL              *LinuxIo;
  LINUX_SERIAL_IO_PRIVATE_DATA       *Private;
  UINTN                               LinuxHandle;
  ...
  //
  // Check to see if we can access the hardware device. If it's Open in Linux we
  // will not get access.
  //
  EfiUnicodeStrToAsciiStr(LinuxIo->EnvString, AsciiDevName);
  LinuxHandle = LinuxIo->LinuxThunk->Open (AsciiDevName, O_RDWR | O_NOCTTY, 0);
  
  if (LinuxHandle == -1) {
    DEBUG ((EFI_D_INFO, "Failed to open serial device, %s!\r\n", LinuxIo->EnvString ));
    LinuxIo->LinuxThunk->Perror (AsciiDevName);
    Status = EFI_DEVICE_ERROR;
    goto Error;
  }
  DEBUG ((EFI_D_INFO, "Success to open serial device %s, Hanle = 0x%x \r\n", LinuxIo->EnvString, LinuxHandle));
  ...
  Status = gBS->AllocatePool (
                  EfiBootServicesData,
                  sizeof (LINUX_SERIAL_IO_PRIVATE_DATA),
                  &Private
                  );
  if (EFI_ERROR (Status)) {
    goto Error;
  ...
  //
  Private->Signature              = LINUX_SERIAL_IO_PRIVATE_DATA_SIGNATURE;
  Private->LinuxHandle             = LinuxHandle;
  ...
  EfiLibAddUnicodeString (
    "eng",
    gLinuxSerialIoComponentName.SupportedLanguages,
    &Private->ControllerNameTable,
    LinuxIo->EnvString
    );

  Private->SerialIo.Revision      = SERIAL_IO_INTERFACE_REVISION;
  Private->SerialIo.Reset         = LinuxSerialIoReset;
  Private->SerialIo.SetAttributes = LinuxSerialIoSetAttributes;
  Private->SerialIo.SetControl    = LinuxSerialIoSetControl;
  Private->SerialIo.GetControl    = LinuxSerialIoGetControl;
  Private->SerialIo.Write         = LinuxSerialIoWrite;
  Private->SerialIo.Read          = LinuxSerialIoRead;
  ...
  if (RemainingDevicePath != NULL) {
    ...
    // from the LinuxIo handle. The Uart setings are zero here, since
...
Error:
  ...
  if (Private != NULL) {
    if (Private->Handle != NULL) {
      ...
      if (LinuxHandle != -1) {
        Private->LinuxThunk->Close (LinuxHandle);
      }
...
--*/
{
  ...
  for (Index = 0; Index < NumberOfChildren; Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      Private = LINUX_SERIAL_IO_PRIVATE_DATA_FROM_THIS (SerialIo);
      ...
      if (EFI_ERROR (Status)) {
        ...
        Private->LinuxThunk->Close (Private->LinuxHandle);
...
--*/
{
  ...
  Private = LINUX_SERIAL_IO_PRIVATE_DATA_FROM_THIS (This);

  LinuxStatus  = Private->LinuxThunk->Tcflush (
                                    Private->LinuxHandle, 
                                    TCIOFLUSH
                                    );
  switch (LinuxStatus) {
...
// TODO:    EFI_SUCCESS - add return value to function comment
{
  ...
  // 
  if (Private->LinuxThunk->Tcgetattr (Private->LinuxHandle, &Private->LinuxTermios) == -1) {
    Private->LinuxThunk->Perror ("IoSetAttributes");
    gBS->RestoreTPL (Tpl);
    return EFI_DEVICE_ERROR;
  }
  ...
  // Setting Baud Rate
  // 
  Private->LinuxThunk->Cfsetispeed (&Private->LinuxTermios, ConvertBaud2Linux(BaudRate));
  Private->LinuxThunk->Cfsetospeed (&Private->LinuxTermios, ConvertBaud2Linux(BaudRate));
  //
  // Setting DataBits 
  // 
  Private->LinuxTermios.c_cflag &= ~CSIZE;
  Private->LinuxTermios.c_cflag |= ConvertByteSize2Linux (DataBits);
  //
  // Setting Parity
  // 
  ConvertParity2Linux (&Private->LinuxTermios, Parity);
  //
  // Setting StopBits
  // 
  ConvertStopBit2Linux (&Private->LinuxTermios, StopBits);
  //
  // Raw input
  // 
  Private->LinuxTermios.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG);
  //
  // Raw output
  // 
  Private->LinuxTermios.c_oflag &= ~OPOST;
  //
  // Support hardware flow control 
  // 
  Private->LinuxTermios.c_cflag &= ~CRTSCTS;;
  //
  // Time out
  // 
  Private->LinuxTermios.c_cc[VMIN]  = 0;
  Private->LinuxTermios.c_cc[VTIME] = (Timeout/1000000) * 10;

  //
  // Set the options
  // 
  if (-1 == Private->LinuxThunk->Tcsetattr (
                        Private->LinuxHandle, 
                        TCSANOW, 
                        &Private->LinuxTermios
                        )) {
...
--*/
{
  ...
  Result  = Private->LinuxThunk->IoCtl (Private->LinuxHandle, TIOCMGET, (UINTN)&Status);

  if (Result == -1) {
    Private->LinuxThunk->Perror ("SerialSetControl");
    gBS->RestoreTPL (Tpl);
    return EFI_DEVICE_ERROR;
  }
  ...
  Result  = Private->LinuxThunk->IoCtl (Private->LinuxHandle, TIOCMSET, (UINTN)&Status);
...
--*/
{
  ...
  Result  = Private->LinuxThunk->IoCtl (Private->LinuxHandle, TIOCMGET, (UINTN)&Status);
  if (Result == -1) {
    Private->LinuxThunk->Perror ("SerialGetControl");
    gBS->RestoreTPL (Tpl);
    return EFI_DEVICE_ERROR;
  }
  ...
  Result = Private->LinuxThunk->IoCtl (Private->LinuxHandle, FIONREAD, (UINTN)&Bytes);
  if (Result == -1) {
    Private->LinuxThunk->Perror ("SerialGetControl");
    gBS->RestoreTPL (Tpl);
    return EFI_DEVICE_ERROR;
  ...
  if (Private->SoftwareLoopbackEnable || Private->HardwareLoopbackEnable) {
    ...
    do {
      if (Private->HardwareFlowControl) {
        ...
        // Send RTS
        //
        LinuxSerialIoGetControl (&Private->SerialIo, &Control);
        Control |= EFI_SERIAL_REQUEST_TO_SEND;
        LinuxSerialIoSetControl (&Private->SerialIo, Control);
      }
      
      //
      //  Do the write
      //
      BytesWritten = Private->LinuxThunk->Write ( 
                                           Private->LinuxHandle,
...
--*/
{
  ...
  if (Private->SoftwareLoopbackEnable || Private->HardwareLoopbackEnable) {
    ...
    BytesRead = Private->LinuxThunk->Read (Private->LinuxHandle, Buffer, *BufferSize);
    if (Private->HardwareFlowControl) {
      LinuxSerialIoGetControl (&Private->SerialIo, &Control);
