...
#include <fcntl.h>
#include <unistd.h>
#include "LinuxBlockIo.h"

EFI_DRIVER_BINDING_PROTOCOL gLinuxBlockIoDriverBinding = {
...
EFI_DRIVER_ENTRY_POINT (InitializeLinuxBlockIo)
...
// TODO:    SystemTable - add argument and description to function comment
{
  return EfiLibInstallAllDriverProtocols (
          ImageHandle,
          SystemTable,
          &gLinuxBlockIoDriverBinding,
          ImageHandle,
          &gLinuxBlockIoComponentName,
          &gLinuxBlockIoDriverConfiguration,
          &gLinuxBlockIoDriverDiagnostics
          );
}
...
// TODO:    RemainingDevicePath - add argument and description to function comment
{
  ...
  //
  // Make sure the LinuxThunkProtocol is valid
  //
  ...
  if (LinuxIo->LinuxThunk->Signature == EFI_LINUX_THUNK_PROTOCOL_SIGNATURE) {
    ...
    //
    if (EfiCompareGuid (LinuxIo->TypeGuid, &gEfiLinuxVirtualDisksGuid)) {
      Status = EFI_SUCCESS;
    }
  }
...
// TODO:    RemainingDevicePath - add argument and description to function comment
{
  ...
  //
  Status = LinuxBlockIoCreateMapping (
				      LinuxIo,
				      Handle,
				      Buffer,
				      WriteProtected,
...
--*/
{
  ...
  Private = LINUX_BLOCK_IO_PRIVATE_DATA_FROM_THIS (BlockIo);
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    Private->LinuxThunk->Close (Private->fd);
...
--*/
{
  ...
  Private->Signature  = LINUX_BLOCK_IO_PRIVATE_SIGNATURE;
  ...
  EfiLibAddUnicodeString (
    "eng",
    gLinuxBlockIoComponentName.SupportedLanguages,
  ...
  BlockIo->Reset = LinuxBlockIoResetBlock;
  BlockIo->ReadBlocks = LinuxBlockIoReadBlocks;
  BlockIo->WriteBlocks = LinuxBlockIoWriteBlocks;
  BlockIo->FlushBlocks = LinuxBlockIoFlushBlocks;
  ...
  Status              = LinuxBlockIoOpenDevice (Private);
  if (!EFI_ERROR (Status)) {
...
--*/
{
  ...
  // Open the device
  //
  Private->fd = Private->LinuxThunk->Open
  ...
  if (FileSize != EndOfFile) {
    ...
    // first set it to 0
    //
    Private->LinuxThunk->Ftruncate (Private->fd, 0);

    //
    // then set it to the needed file size (OS will zero fill it)
    //
    Private->LinuxThunk->Ftruncate (Private->fd, EndOfFile);
  }
...
--*/
{
  ...
  switch (Private->LinuxThunk->GetLastError ()) {
...
--*/
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    return LinuxBlockIoError (Private);
  }
...
--*/
{
  ...
  Private = LINUX_BLOCK_IO_PRIVATE_DATA_FROM_THIS (This);

  Status  = LinuxBlockIoReadWriteCommon (Private, MediaId, Lba, BufferSize, Buffer, "LinuxReadBlocks");
  if (EFI_ERROR (Status)) {
    return Status;
  }

  len = Private->LinuxThunk->Read (Private->fd, Buffer, BufferSize);
  if (len != BufferSize) {
...
--*/
{
  ...
  Status  = LinuxBlockIoReadWriteCommon (Private, MediaId, Lba, BufferSize, Buffer, "LinuxWriteBlocks");
  if (EFI_ERROR (Status)) {
    return Status;
  }

  len = Private->LinuxThunk->Write (Private->fd, Buffer, BufferSize);
  if (len != BufferSize) {
...
// TODO:    MoveMethod - add argument and description to function comment
{
  ...
  res = Private->LinuxThunk->Lseek(Private->fd, DistanceToMove, MoveMethod);
  if (res == -1) {
