...
--*/
{
  ...
  Status = Private->LinuxThunk->UgaCreate(&Private->UgaIo, Private->WindowName);
  return Status;
}
...
// TODO:    EFI_SUCCESS - add return value to function comment
{
  ...
  Private->UgaDraw.GetMode        = LinuxUgaGetMode;
  Private->UgaDraw.SetMode        = LinuxUgaSetMode;
  Private->UgaDraw.Blt            = LinuxUgaBlt;

  Private->HardwareNeedsStarting  = TRUE;
  Private->UgaIo = NULL;

  LinuxUgaInitializeSimpleTextInForWindow (Private);
...
{
  EFI_STATUS  Status;
  Status = LinuxUgaDestructor (Context);
}
