...
--*/
{
  ...
  stat = MapIt (
          ...
          TO_AND_FROM_DEVICE,
          (UINT64) &AdapterInfo->Mapped_MemoryPtr
          );
...
--*/
{
  ...
  while ((status = rx_ptr->cb_header.status) & RX_COMPLETE) {
    ...
    if (pkt_len != 0) {
      ...
      EfiCopyMem ((INT8 *) (UINTN) rx_cpbptr->BufferAddr, (INT8 *) &rx_ptr->RFDBuffer, Tmp_len);
...
--*/
{
  ...
  EfiCopyMem ((VOID *) (UINTN) DBaddr, (VOID *) &db, (UINTN) DBsize);
