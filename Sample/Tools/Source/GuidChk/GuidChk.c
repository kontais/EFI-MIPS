  ...
  )
{
  ...
  if ((DirectoryName != NULL) && (strlen (DirectoryName) > 0)) {
    strcat (DirPath, "/");
    strcat (DirPath, DirectoryName);
  }

  //strcat (DirPath, "\\*.*");    no support in SysLib.c

  //
  // Start file searching for files and directories
  //
  FileSearchStart (&FSData, DirPath, FILE_SEARCH_FILE | FILE_SEARCH_DIR);
  ...
  while (!Done) {
    ...
    if (FSData.FileFlags & FILE_SEARCH_DIR) {
      ...
      //
      if (!NoSubdirs) {
        ProcessDirectory (DirPath, FSData.FileName);
      }
    } else if (FSData.FileFlags & FILE_SEARCH_FILE) {
      //
      // printf ("file\n");
      //
      ProcessFile (DirPath, FSData.FileName);
    } else {
      //
      // printf ("unknown\n");
      //
