  ...
  )
{
  ...
  // Begin the search
  //
  FSData->DirHandle = FindFirstFile (DirPath, &(FSData->FindData));
  if (FSData->DirHandle == NULL) {
    return STATUS_ERROR;
  }
  //
  ...
  while (!Done) {
    ...
    // Go on to next file
    //
    if (!FindNextFile (FSData->DirHandle, &(FSData->FindData))) {
