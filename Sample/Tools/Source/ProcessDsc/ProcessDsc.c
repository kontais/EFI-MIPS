...
// example, when a ".c" is built, it results in a ".o" file.
//
...
--*/
{
  ...
  if (OverridePath != NULL) {
    fprintf (MakeFptr, "INC += -I %s\n", OverridePath);
    fprintf (MakeFptr, "INC += -I %s/%s \n", OverridePath, Processor);
  }
  //
    ...
    while (DSCFileGetLine (ComponentFile, Str, sizeof (Str)) != NULL) {
      ...
      if (*Cptr) {
        ...
        if (*Cptr == '.') {
          ...
          //
          if (Cptr[1] == 0) {
            fprintf (MakeFptr, "INC += -I $(SOURCE_DIR)\n");
            fprintf (
              MakeFptr,
              "INC += -I $(SOURCE_DIR)/%s \n",
              Processor
            ...
            fprintf (
              MakeFptr,
              "INC += -I $(SOURCE_DIR)/%s/%s \n",
              Cptr,
              Processor
              );
          }
        } else if ((Cptr[1] != ':') && isalpha (*Cptr)) {
          fprintf (MakeFptr, "INC += -I $(EFI_SOURCE)/%s \n", Cptr);
          fprintf (
            MakeFptr,
            "INC += -I $(EFI_SOURCE)/%s/%s \n",
            Cptr,
            Processor
            );
        } else {
...
Arguments:
  ...
  MySourceFile.c   BUILT_NAME=$(BUILD_DIR)\MySourceFile.o
    ...
    while (DSCFileGetLine (ComponentFile, Str, sizeof (Str)) != NULL) {
      ...
      if (*Cptr) {
        ...
        if (Mode & SOURCE_MODE_SOURCE_FILES) {
          ...
          if (OverridePath != NULL) {
            //
            // SOURCE_FILES += c:\Path\ThisFile.c
            //
            fprintf (MakeFptr, "SOURCE_FILES += %s\n", TempFileName);
            ...
            //
            // SOURCE_FILES += $(SOURCE_DIR)\ThisFile.c
            //
            fprintf (MakeFptr, "SOURCE_FILES += $(SOURCE_DIR)/%s\n", FileName);
          }
  ...
  if (TempSect != NULL) {
    while (DSCFileGetLine (ComponentFile, Str, sizeof (Str)) != NULL) {
      ...
      if (*Cptr) {
        ...
        // the built version of the file is. For example, .c -> .o.
        //
  ...
  )
{
  ...
  if (Cptr != NULL) {
    if (_strnicmp (Cptr, FileName, strlen (Cptr)) == 0) {
      ...
      if ((mkdir (BuildDir,S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0) && (errno != EEXIST)) {
        Cptr = FileName;
      } else {
        //
...
{
  //
  // If the first character is /, then it is an absolute path.
  //
  if (FileName[0] == '/') {
    return 1;
  }
