...
#include <string.h>
#include <stdlib.h>
#include <uuid/uuid.h> // for uuid_generate
...
UINT8                   Mips32ResetVectorDataArray[SIZEOF_STARTUP_DATA_ARRAY] = {
  //
  // 3C 02 XX XX        ; lui   v0, 0x8765
  // 34 42 XX XX        ; ori   v0, v0, 0x4321
  // 00 40 F8 09        ; jalr  v0
  // 00 00 00 00        ; nop
  //
  0x00,
...
--*/
{
  ...
  for (Index = 1;; Index++) {
    ...
    ImageContext.ImageRead  = (EFI_PEI_ELF_LOADER_READ_FILE) FfsRebaseImageRead;
    ...
    ImageContext.DestinationAddress = NewPe32BaseAddress + sizeof (EFI_PE32_SECTION);
    ...
    if (ImageSize > (Pe32FileSize - sizeof(EFI_PE32_SECTION))) {
      Error (
        NULL,
        0,
        0,
    ...
    memcpy ((VOID *) ((UINTN) CurrentPe32Section.Pe32Section + sizeof (EFI_PE32_SECTION)),
            (VOID *) MemoryImagePointerAligned, 
            (Pe32FileSize - sizeof(EFI_PE32_SECTION)));

    //
    // 这儿需要重新计算Checksum
    //
    ChecksumFfsFile(FfsFile);
...
--*/
{
  ...
  if (Status == EFI_SUCCESS) {
    Status = GetPe32Info (
               (VOID *) ((UINTN) Pe32Section.Pe32Section + sizeof (EFI_PE32_SECTION)),
               &EntryPoint,
               &BaseOfCode,
               &MachineType
               );
  ...
  if (IsVtfFile ((EFI_FFS_FILE_HEADER *) FileBuffer)) {
    if ((UINTN) *VtfFileImage == (UINTN) FvImage->Eof) {
      ...
      *(UINT32 *) ((*VtfFileImage)->Size) = FileSize;
      ...
      //
#ifndef EFI_LINUX_EMULATOR
      CurrentFileBaseAddress = FvInfo->BaseAddress + ((UINTN) (*VtfFileImage) - (UINTN) FvImage->FileImage);
      Status = RebaseFfsFile ((*VtfFileImage), CurrentFileBaseAddress);
      if (EFI_ERROR(Status)) {
        free (FileBuffer);
...
--*/
{
  ...
  PeiCorePhysicalAddress += (UINTN) Pe32Section.Pe32Section + sizeof (EFI_PE32_SECTION) - (UINTN) FvImage->FileImage;
  ...
  if (MachineType == EFI_IMAGE_MACHINE_IA64) {
    ...
    // Physical address is FV base + offset of PE32 + offset of the entry point
    //
    SecCorePhysicalAddress = (EFI_PHYSICAL_ADDRESS) EntryPoint;
    ...
    //
    // Update the FVH header block ZeroVector region.
    //
    BytePointer2  = Mips32ResetVectorDataArray;
	*(UINT16*) &BytePointer2[0] |= (UINT16)(EntryPoint >> 16);
	*(UINT16*) &BytePointer2[4] |= (UINT16)(EntryPoint & 0x0000FFFF);
...
--*/
{
  ...
  if (Elf32Hdr->e_ident[EI_MAG0] != ELFMAG0 ||
      ...
      printf ("ERROR: Unknown ELF Signature, 0x%04X.\n", *(UINT32*)&Elf32Hdr->e_ident[0]);
  ...
  if (Elf32Hdr->e_ident[EI_CLASS] != ELFCLASS32) {
  	printf ("ERROR: We only support ELF32 image now.\n");
    return EFI_UNSUPPORTED;
  }

  if (Elf32Hdr->e_machine != EFI_IMAGE_MACHINE_MIPS) {
  	printf ("ERROR: We only support MIPS arch now.\n");
  	return EFI_UNSUPPORTED;
  }

  *EntryPoint   = (UINT32) Elf32Hdr->e_entry;
  *BaseOfCode   = 0;
  *MachineType  = (UINT16) Elf32Hdr->e_machine;
  ...
  *SymImage = malloc (SYMBOL_FILE_SIZE);
  ...
  if ((UINTN) VtfFileImage != (UINTN) FvImageMemoryFile.Eof) {
    ...
    //if ((FvInfo.BaseAddress + FvInfo.Size) == FV_IMAGES_TOP_ADDRESS) {       
      ...
      //
      //  Because MIPS use all ZeroVector, so must update FV Header checksum
      //
      FvHeader->Checksum      = 0;
      FvHeader->Checksum      = CalculateChecksum16 ((UINT16 *) FvHeader, FvHeader->HeaderLength / sizeof (UINT16));
    //}
