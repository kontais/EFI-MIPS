  ...
  for (Lang = mDBData.LanguageList; Lang != NULL; Lang = Lang->Next) {
    ...
    if (LanguagesOfInterest != NULL) {
      ...
      for (LOIPtr = LanguagesOfInterest; LOIPtr != NULL; LOIPtr = LOIPtr->Next) {
        if (_wcsncmp (LOIPtr->Str, Lang->LanguageName, LANGUAGE_IDENTIFIER_NAME_LEN) == 0) {
          LangName    = LOIPtr->Str;
          LanguageOk  = TRUE;
          break;
        }
    ...
    fprintf (
      ...
      UnicodeToAscii(Lang->LanguageName),
      UnicodeToAscii(Lang->PrintableLanguageName)
      );
    ...
    StringPack.PrintableLanguageName = (STRING_OFFSET) (StringPack.LanguageNameString + (_wcslen (LangName) + 1) * sizeof (WCHAR));
    //
    ...
    for (StringIndex = 0; StringIndex < mDBData.NumStringIdentifiersReferenced; StringIndex++) {
      ...
      if (StringIndex == STRING_ID_LANGUAGE_NAME) {
        Len += (_wcslen (LangName) + 1) * sizeof (WCHAR);
      } else {
        //
    ...
    for (StringIndex = 0; StringIndex < mDBData.NumStringIdentifiersReferenced; StringIndex++) {
      ...
      fprintf (Fptr, " // offset to string %s (0x%04X)", UnicodeToAscii(StringIdentifier->StringName), StringIndex);
      //
      // For the first string (language name), we print out the "spacat" if they
      // requested it. We set LangName to point to the proper language name string above.
      //
      if (StringIndex == STRING_ID_LANGUAGE_NAME) {
        Offset += (_wcslen (LangName) + 1) * sizeof (WCHAR);
        CurrString = StringDBFindString (
                      Lang->LanguageName,
                      StringIdentifier->StringName,
                      NULL, // scope
      ...
      } else if (_wcscmp (CurrString->LanguageName, Lang->LanguageName) != 0) {
        fprintf (
          Fptr,
    ...
    while (StringIndex < mDBData.NumStringIdentifiers) {
      StringIdentifier = StringDBFindStringIdentifierByIndex (StringIndex);
      if (StringIdentifier != NULL) {
        fprintf (Fptr, "  // %s not referenced\n", UnicodeToAscii(StringIdentifier->StringName));
      }
    ...
    for (StringIndex = 0; StringIndex < mDBData.NumStringIdentifiersReferenced; StringIndex++) {
      ...
      fprintf (Fptr, "  // string %s offset 0x%08X\n  ", UnicodeToAscii(StringIdentifier->StringName), Offset);
      //
      // For the first string (language name), we print out the "spacat" if they
      // requested it. We set LangName to point to the proper language name string above.
      //
  ...
  )
{
  ...
  while (Identifier != NULL) {
    ...
    if (Identifier->Flags & STRING_FLAGS_REFERENCED) {
      fprintf (Fptr, "#define %-40s 0x%04X\n", UnicodeToAscii(Identifier->StringName), Identifier->Index);
    } else {
  ...
  )
{
  ...
  StringIdentifier->StringName = (WCHAR *) malloc ((_wcslen (StringName) + 1) * sizeof (WCHAR));
  if (StringIdentifier->StringName == NULL) {
    Error (NULL, 0, 0, NULL, "memory allocation error");
    return STATUS_ERROR;
  }

  _wcscpy (StringIdentifier->StringName, StringName);
  ...
  )
{
  ...
  if (LanguageName != NULL) {
    ...
    if (Size != 3) {
      ...
      if (Size > 3) {
        ...
        // on the length.
        //
        _wcscpy (TempLangName, LanguageName);
        for (; Size < 3; Size++) {
          TempLangName[Size] = L'?';
        }
  ...
  if (StringDBFindString (Lang->LanguageName, StringName, Scope, NULL, NULL) != NULL) {
    if ((_wcscmp (StringName, LANGUAGE_NAME_STRING_NAME) == 0) &&
        (_wcscmp (StringName, PRINTABLE_LANGUAGE_NAME_STRING_NAME) == 0)
        ) {
      ParserError (
        0,
  ...
  Size              = (_wcslen (String) + 1) * sizeof (WCHAR);
  Str->Flags        = Flags;
  Str->Scope        = Scope;
  ...
  //
  Str->Size = (_wcslen (Str->Str) + 1) * sizeof (WCHAR);
  if (Lang->String == NULL) {
    Lang->String = Str;
  } else {
  ...
  )
{
  ...
  while (Lang != NULL) {
    if (_wcscmp (LanguageName, Lang->LanguageName) == 0) {
      break;
    }
  ...
  )
{
  ...
  if (Lang != NULL) {
    ...
    //
    if (_wcscmp (PrintableLanguageName, Lang->PrintableLanguageName) != 0) {
      ParserError (
        0,
    ...
    //
    _wcscpy (Lang->LanguageName, LanguageName);
    Lang->PrintableLanguageName = (WCHAR *) malloc ((_wcslen (PrintableLanguageName) + 1) * sizeof (WCHAR));
    if (Lang->PrintableLanguageName == NULL) {
      Error (NULL, 0, 0, NULL, "memory allocation error");
      return STATUS_ERROR;
    }

    _wcscpy (Lang->PrintableLanguageName, PrintableLanguageName);
  ...
  )
{
  ...
  Identifier = mDBData.StringIdentifier;
  while (Identifier != NULL) {
    if (_wcscmp (StringName, Identifier->StringName) == 0) {
      return Identifier;
    }
  ...
  )
{
  ...
  while (*From) {
    if (*From == UNICODE_BACKSLASH) {
      ...
      //
      if (_wcsncmp (From, UNICODE_WIDE_STRING, sizeof (UNICODE_WIDE_STRING) / sizeof (WCHAR) - 1) == 0) {
        *To = WIDE_CHAR;
        From += sizeof (UNICODE_WIDE_STRING) / sizeof (WCHAR) - 2;
      } else if (_wcsncmp (From, UNICODE_NARROW_STRING, sizeof (UNICODE_NARROW_STRING) / sizeof (WCHAR) - 1) == 0) {
        //
        // Found: \narrow
        //
        *To = NARROW_CHAR;
        From += sizeof (UNICODE_NARROW_STRING) / sizeof (WCHAR) - 2;
      } else if (_wcsncmp (From, UNICODE_NBR_STRING, sizeof (UNICODE_NBR_STRING) / sizeof (WCHAR) - 1) == 0) {
        //
        // Found: \nbr
        //
        *To = NON_BREAKING_CHAR;
        From += sizeof (UNICODE_NBR_STRING) / sizeof (WCHAR) - 2;
      } else if (_wcsncmp (From, UNICODE_BR_STRING, sizeof (UNICODE_BR_STRING) / sizeof (WCHAR) - 1) == 0) {
        //
  ...
  )
{
  ...
  for (Counter = 0; Counter < mDBData.NumStringIdentifiers; Counter++) {
    StrLen = _wcslen (StringIdentifier->StringName) + 1;
  ...
  )
{
  ...
  _swprintf (WName, (strlen (StringIdentifierName) + 1) * sizeof (WCHAR), L"%S", AsciiToUnicode(StringIdentifierName,NULL));
  ...
  )
{
  ...
  OutFptr         = fopen (WOutputFileName, "wb");
  ...
  // "/=#" to the output file.
  //
  _swprintf (Line, _wcslen(Line) * sizeof (WCHAR), L"/=#");
  fwrite (Line, _wcslen (Line) * sizeof (WCHAR), 1, OutFptr);
  ...
  for (Lang = mDBData.LanguageList; Lang != NULL; Lang = Lang->Next) {
    fwrite (&CrLf, sizeof (CrLf), 1, OutFptr);
    _swprintf (Line, _wcslen(Line) * sizeof (WCHAR), L"#langdef %s \"%s\"", Lang->LanguageName, Lang->PrintableLanguageName);
    fwrite (Line, _wcslen (Line) * sizeof (WCHAR), 1, OutFptr);
    fwrite (&CrLf, sizeof (CrLf), 1, OutFptr);
    fwrite (&CrLf, sizeof (CrLf), 1, OutFptr);
    //
    ...
    for (StrList = Lang->String; StrList != NULL; StrList = StrList->Next) {
      ...
      //
      _swprintf (Line, _wcslen(Line) * sizeof (WCHAR), L"// flags=0x%02X", (UINT32) StrList->Flags);
      fwrite (Line, _wcslen (Line) * sizeof (WCHAR), 1, OutFptr);
      fwrite (&CrLf, sizeof (CrLf), 1, OutFptr);
      //
      // Print the scope if changed
      //
      if ((Scope == NULL) || (_wcscmp (Scope, StrList->Scope) != 0)) {
        _swprintf (Line, _wcslen(Line) * sizeof (WCHAR), L"#scope %s", StrList->Scope);
        fwrite (Line, _wcslen (Line) * sizeof (WCHAR), 1, OutFptr);
        fwrite (&CrLf, sizeof (CrLf), 1, OutFptr);
        Scope = StrList->Scope;
      }
  ...
  )
{
  ...
  if (Str == NULL) {
    ...
    Flags = 0;
    Size  = (UINT16) ((_wcslen (Str) + 1) * sizeof (WCHAR));
  }
  ...
  )
{
  ...
  if (IndirectionList != NULL) {
    for (IndListPtr = IndirectionList; IndListPtr != NULL; IndListPtr = IndListPtr->Next) {
      if (_wcscmp (StringName, IndListPtr->Str1) == 0) {
  ...
  for (Lang = mDBData.LanguageList; Lang != NULL; Lang = Lang->Next) {
    if (_wcscmp (LanguageName, Lang->LanguageName) == 0) {
      ...
      for (CurrString = Lang->String; CurrString != NULL; CurrString = CurrString->Next) {
        if (_wcscmp (StringName, CurrString->StringName) == 0) {
          //
          // Found a string name match. See if we're supposed to find
          // a scope match.
          //
          if (Scope != NULL) {
            if (_wcscmp (CurrString->Scope, Scope) == 0) {
              return CurrString;
            }
          } else {
            return CurrString;
  ...
  while (LanguagesOfInterest != NULL) {
    ...
    //
    if (_wcsncmp (LanguageName, LanguagesOfInterest->Str, LANGUAGE_IDENTIFIER_NAME_LEN) == 0) {
      ...
      while (*WCharPtr) {
        ...
        // command line.
        //
        if (_wcslen (WCharPtr) < LANGUAGE_IDENTIFIER_NAME_LEN) {
          Error (NULL, 0, 0, "malformed alternate language list", "%S", LanguagesOfInterest->Str);
          return NULL;
        }

        _wcsncpy (TempLangName, WCharPtr, LANGUAGE_IDENTIFIER_NAME_LEN);
  ...
  )
{
  ...
  NewStr = MALLOC ((_wcslen (Str) + 1) * sizeof (WCHAR));
  if (NewStr == NULL) {
