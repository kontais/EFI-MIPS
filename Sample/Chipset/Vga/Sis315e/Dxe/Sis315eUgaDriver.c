...
EFI_DRIVER_BINDING_PROTOCOL gSis315eUgaDriverBinding = {
...
EFI_DRIVER_ENTRY_POINT  (InitializeSis315eUga);

EFI_STATUS
EFIAPI
InitializeSis315eUga(
...
// TODO:    SystemTable - add argument and description to function comment
{
  return EfiLibInstallAllDriverProtocols (
          ImageHandle,
          SystemTable,
          &gSis315eUgaDriverBinding,
          ImageHandle,
          &gSis315eUgaComponentName,
          NULL,
          NULL
          );
}
...
// TODO:    RemainingDevicePath - add argument and description to function comment
{
  ...
  Status = Sis315eUgaSupported (LinuxIo);
  //
...
//  Private->LinuxThunk           = LinuxIo->LinuxThunk;
  ...
  EfiLibAddUnicodeString (
    "eng",
    gSis315eUgaComponentName.SupportedLanguages,
    &Private->ControllerNameTable,
    LinuxIo->EnvString
    );

//  Private->WindowName = LinuxIo->EnvString;
  DEBUG((EFI_D_ERROR,"Sis315eUgaDriverBindingStart\n"));

  Status              = Sis315eUgaConstructor (Private);
...
// TODO:    EFI_DEVICE_ERROR - add return value to function comment
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    Status = Sis315eUgaDestructor (Private);
    if (EFI_ERROR (Status)) {
