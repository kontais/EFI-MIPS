...
--*/
{
  ...
  if (NotifyFunction == NULL) {
    /* Disable timer.  */
    gLinux->SetTimer (0, TimerCallback);
  } else if (mTimerNotifyFunction == NULL) {
    /* Enable Timer.  */
    gLinux->SetTimer (mTimerPeriod * 10, TimerCallback);
  }
...
--*/
{
  ...
  if (TimerPeriod == 0
    ...
    gLinux->SetTimer (TimerPeriod * 10, TimerCallback);
  }
...
EFI_DRIVER_ENTRY_POINT (LinuxTimerDriverInitialize)
