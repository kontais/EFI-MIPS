...
/*++
  ...
  Get the IO Port Mapping.
  
--*/
{
  *IoPortMapping     = PHYS_TO_UNCACHED(BONITO_PCIIO_BASE); // Not Use this, use gCpuIo
  *MemoryPortMapping = PHYS_TO_UNCACHED(BONITO_PCILO_BASE);
  ...
  )
{
  ...
  if (!UsePciExpressAccess) {
    ...
    BonitoRootBridgeIoPciRW (Write, Width, UserAddress, Count, UserBuffer);
