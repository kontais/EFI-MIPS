...
Abstract:
  Tiano PEIM to abstract memory auto-scan in a Linux environment.
...
EFI_STATUS
EFIAPI
PeimInitializeLinuxAutoScanPei (
  IN EFI_FFS_FILE_HEADER       *FfsHeader,
  IN EFI_PEI_SERVICES          **PeiServices
  );

EFI_PEIM_ENTRY_POINT (PeimInitializeLinuxAutoScanPei)
...
--*/
{
  ...
  PEI_LINUX_AUTOSCAN_PPI      *PeiNtService;
  UINT64                      MemorySize;
  ...
  Index = 0;
  do {
    Status = PeiNtService->LinuxAutoScan (Index, &MemoryBase, &MemorySize);
    if (!EFI_ERROR (Status)) {
      Attributes =
        (
