...
--*/
{
  ...
  //
  // Install notify function to fetch memory data through LinuxIo protocol and store to data hub.
  //
...
--*/
{
  ...
  for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++) {
    ...
    if ((LinuxIo->LinuxThunk->Signature == EFI_LINUX_THUNK_PROTOCOL_SIGNATURE) &&
        EfiCompareGuid (LinuxIo->TypeGuid, &gEfiLinuxMemoryGuid)
          ) {
      //
      ...
      TotalMemorySize                       = (UINT64) Atoi (LinuxIo->EnvString);
...
/* eof - MiscSubclassDriverEntryPoint.c */
