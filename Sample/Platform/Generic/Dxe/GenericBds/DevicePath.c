...
EFI_GUID  mEfiLinuxThunkProtocolGuid                 = EFI_LINUX_THUNK_PROTOCOL_GUID;
EFI_GUID  mEfiLinuxGopGuid                           = EFI_LINUX_GOP_GUID;
EFI_GUID  mEfiLinuxSerialPortGuid                    = EFI_LINUX_SERIAL_PORT_GUID;
...
--*/
{
  ...
  case HARDWARE_DEVICE_PATH:
    ...
    //
    if (EfiCompareGuid (&Vendor->Guid, &mEfiLinuxThunkProtocolGuid)) {
      CatPrint (Str, L"%s", L"LinuxBus");
      return ;
    } else if (EfiCompareGuid (&Vendor->Guid, &mEfiLinuxGopGuid)) {
      CatPrint (Str, L"%s", L"GOP");
      return ;
    } else if (EfiCompareGuid (&Vendor->Guid, &mEfiLinuxSerialPortGuid)) {
