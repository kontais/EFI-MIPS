    ...
    )
{
    ...
    Status = BS->HandleProtocol (ImageHandle,&LoadedImageProtocol,(VOID**)&FileImage.LoadedImage);
    if (EFI_ERROR(Status)) {
        Print (L"Could not obtain Loaded Image Protocol\n");
        return EFI_LOAD_ERROR;
    }
    if (FileImage.LoadedImage->DeviceHandle != NULL) {
        Status = BS->HandleProtocol (FileImage.LoadedImage->DeviceHandle,&DevicePathProtocol,(VOID**)&FileImage.DevicePath);
        if (EFI_ERROR(Status)) {
            Print (L"Could not obtain Device Path Protocol\n");
            return EFI_LOAD_ERROR;
        }

        Status = BS->HandleProtocol (FileImage.LoadedImage->DeviceHandle,&FileSystemProtocol,(VOID**)&FileImage.Vol);
        if (EFI_ERROR(Status)) {
        ...
        Status = LibDevicePathToInterface (&FileSystemProtocol, DevicePath,(VOID**)&FileImage.Vol);
    ...
    ) 
{
    ...
    if (EFI_ERROR(Status)) {
        Status = FileImage.CurrentDir->Open (FileImage.CurrentDir,&FileImage.FileHandle,FileImage.FileName,EFI_FILE_MODE_READ,0);
        if (EFI_ERROR(Status)) {
