...
/* bsd db common array size */
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    #define BSDDB_COM_ARSIZE 256 
#else
    #define BSDDB_COM_ARSIZE 4096
#endif
...
bsddb_subscript(bsddbobject *dp, PyObject *key)
{
	int status;
	DBT krec, drec;
	char *data,buf[BSDDB_COM_ARSIZE];
	int size;
	PyObject *result;
	recno_t recno;
...
bsddb_keys(bsddbobject *dp)
{
	...
	DBT krec, drec;
	char *data=NULL,buf[BSDDB_COM_ARSIZE];
	int status;
	int err;
...
bsddb_seq(bsddbobject *dp, int sequence_request)
{
	int status;
	DBT krec, drec;
	char *kdata=NULL,kbuf[BSDDB_COM_ARSIZE];
	char *ddata=NULL,dbuf[BSDDB_COM_ARSIZE];
	PyObject *result;
...
#if !defined(EFI32) && !defined(EFI64) && !defined(EFIX64) 
    // The string keys Python uses don't work very well as
    // 32-bit unsigned numbers, which is how the record number
    // code is going to interpret them. I'm disabling this rather
    // than leave the door to this trap open.
...
struct _inittab Efi_InitTab = { "bsddb", initbsddb185 };
#endif
