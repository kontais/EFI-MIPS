...
char *inet_ntop_ret_global = 0;

const char *
PyPrivate_inet_ntop(int af, const void *src, char *dst, socklen_t size)
{
	if (af == AF_INET) {
		struct in_addr packed_addr;
		if (size < 16)
		...
		inet_ntop_ret_global = strncpy(dst, inet_ntoa(packed_addr), size);
		return inet_ntop_ret_global; 
	}
	/* Should set errno to EAFNOSUPPORT */
	return NULL;
}
