...
{
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
// Under EFI stat() only works on open files so might as well use open()   
    int fd;
    int returnval;

    fd = open(filename, O_RDONLY);
    if (fd < 0) {
        // fprintf(stderr, "isfile: '%s' - open failed\n", filename);
        returnval = 0;
    } else {
        // fprintf(stderr, "isfile: '%s' - opened\n", filename);
        close(fd);
        returnval = 1;
    }
    return returnval;
    ...
    */
    ...
    if (!(pfound = search_for_prefix(argv0_path, home))) {
        if (!Py_FrozenFlag)
            fprintf(stderr,
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    	   "Could not find platform independent libraries in '%s'\n", PYTHONPATH);
#else
