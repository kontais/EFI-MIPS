 ...
 *
 * Portions Copyright (c) by Intel Corporation.
 *
 ...
 *
 * $Id: regexpr.c,v 1.1.1.1 2006/05/30 06:06:48 hhzhou Exp $ */
...
{ \
	int i; \
    state = (match_state *) malloc(sizeof(match_state)); \
	for (i = 0; i < nregs; i++) \
	{ \
		state->start[i] = NULL; \
		state->end[i] = NULL; \
             ...
             regexp_registers_t old_regs)
{
	...
	int regsize;
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    // Don't put big things on the stack
    match_state *state;
#else
	match_state state;
#endif
