...
file_readlines(PyFileObject *f, PyObject *args)
{
	...
	PyObject *line;
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    // Don't put something as big as SMALLCHUNK on the stack
    char *small_buffer;
#else
...
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    small_buffer = (char *) malloc(SMALLCHUNK);
#endif
    buffer = small_buffer;
...
file_write(PyFileObject *f, PyObject *args)
{
	...
	errno = 0;
#if defined(EFI32) || defined(EFI64) || defined(EFIX64)
    // fwrite doesn't do the ASCII to Unicode conversion the 
    //  console needs so chedk if it is a tty
    if ( isatty(fileno(f->f_fp)) )
    	n2 = (fputs(s, f->f_fp) == 0) ? n : 0;
	else
#endif
...
#if defined(EFI32) || defined(EFI64) || defined(EFIX64) /* check here */
        // fwrite doesn't do the ASCII to Unicode conversion the 
        //  console needs
        nwritten = (fputs(PyString_AsString(line), f->f_fp) == 0) ? len : 0;
#else
