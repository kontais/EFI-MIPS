  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (Status == EFI_SUCCESS) {
        ...
        PEI_DEBUG((&PrivateData->PS, EFI_D_INFO, "Find %g\n", 
              &DispatchData->CurrentPeimAddress->Name));
          ...
          if (DepexSatisfied (&PrivateData->PS, DispatchData->CurrentPeimAddress)) {
            ...
            if (Status == EFI_SUCCESS) {
              ...
              if (Status != EFI_SECURITY_VIOLATION) {
                ...
                PEI_DEBUG((&PrivateData->PS, EFI_D_INFO, "Invoke %g\n", 
                      &DispatchData->CurrentPeimAddress->Name));
              ...
              if (PrivateData->SwitchStackSignal) {
                ...
                //
                PEI_DEBUG((&PrivateData->PS, EFI_D_INFO, "SwitchCoreStacks %g\n", 
                      &DispatchData->CurrentPeimAddress->Name));
                ...
                PEI_DEBUG((&PrivateData->PS, EFI_D_INFO, "Error %g\n", 
                      &DispatchData->CurrentPeimAddress->Name));
              }
            }
          }
