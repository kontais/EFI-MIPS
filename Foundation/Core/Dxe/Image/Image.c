...
--*/
{
  ...
  Image->ImageContext.ImageRead = (EFI_PEI_ELF_LOADER_READ_FILE) CoreReadImageFile;

  //
  // Get information about the image being loaded.
  //
  Status = gEfiPeiElfLoader->GetImageInfo (gEfiPeiElfLoader, &(Image->ImageContext));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  if (DstBuffer == 0) {
    ...
    if (Image->ImageContext.IsExecutable) {
    ...
    if (Image->ImageContext.IsRelocatable) {
      Status = CoreAllocatePages (
                 AllocateAnyPages,
                 Image->ImageContext.ImageCodeMemoryType,
                 Image->NumberOfPages,
    ...
    Size = EFI_SIZE_TO_PAGES ((UINTN) Image->ImageContext.ImageSize);
...
//                        ~((UINTN) Image->ImageContext.SectionAlignment - 1);
  ...
  //
  // Allocate memory for relocate info 
  //
  if (Image->ImageContext.IsRelocatable) {
  ...
  //
  Status = gEfiPeiElfLoader->LoadImage (gEfiPeiElfLoader, &(Image->ImageContext));
  if (EFI_ERROR (Status)) {
    goto Done;
  }
...
//  }
  ...
  //
  Status = gEfiPeiElfLoader->RelocateImage (gEfiPeiElfLoader, &(Image->ImageContext));
  if (EFI_ERROR (Status)) {
    goto Done;
  }
  ...
  if (Attribute & EFI_LOAD_PE_IMAGE_ATTRIBUTE_RUNTIME_REGISTRATION) {
    if (Image->ImageContext.ImageType == EFI_IMAGE_SUBSYSTEM_EFI_RUNTIME_DRIVER) {
      ...
      Image->RuntimeData->RelocationData = NULL; //Image->ImageContext.FixupData;
  ...
  DEBUG_CODE (
  {
    ...
    DEBUG ((EFI_D_INFO | EFI_D_LOAD, "%a",
            Image->ImageContext.SectionLoadAddress[Image->ImageContext.StringTableIndex] + 1));
...
--*/
{
  ...
  //
  gEfiPeiElfLoader->UnloadImage (&Image->ImageContext);
